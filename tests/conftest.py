...
@pytest.fixture()
def certification_service(container: Container) -> CertificationService:
    """A certification service with a FRESH per-test scorecard store (no cross-test history)."""
    service = build_certification_service(container)
    service._store = ScorecardStore()  # isolate decert/tenant history per test
    return service
...
def api_client() -> Iterator[TestClient]:
    ...
    from data_quality_governance.api.app import app

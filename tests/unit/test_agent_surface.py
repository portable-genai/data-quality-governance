...
from data_quality_governance.agent import (
    SKILLS,
    TOOL_FUNCTIONS,
    agent_card_document,
    build_agent_card,
...
def test_the_card_document_is_json_safe_and_names_the_region() -> None:
    ...
    assert document["name"] == "data-quality-governance"
...
def test_the_agent_path_does_not_manufacture_a_review_for_a_certified_dataset() -> None:
    result = certify_dataset(
...
def test_the_tool_output_carries_no_raw_sampled_personal_data() -> None:
    """P-04 at the agent boundary: a tool result becomes model context, so it is minimised.

    A scorecard reports statistics, categories and citations, never the sampled values, and any
    string that did carry one is masked by the shared pack on the way out.
    """
    result = certify_dataset(sample_cases.CERTIFIED_DATASET, settings=local_settings())
    rendered = repr(result)
    assert "user1001@bank.example" not in rendered, "a sampled email leaked into model context"
    assert "@bank.example" not in rendered or "REDACTED" in rendered

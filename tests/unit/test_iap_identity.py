...
from data_quality_governance.adapters.gcp.identity import (
    _AUDIENCE_ENV,
...
from data_quality_governance.config import Settings, _read_settings_file
from data_quality_governance.ports.identity import (
    VERIFIED,
...
_API_MODULE = "data_quality_governance.api.app"
...
_REBOUND_SETTINGS = "\n".join(
    [
        ...
        *[
        ...
        "  warehouse:",
        *[
            f"    {p}: {_PKG}.adapters.local.warehouse:LocalWarehouseAdapter"
            for p in ("local", "gcp")
        ],
        f"    onprem: {_PKG}.adapters.onprem.warehouse:OnPremWarehouseAdapter",
        "  catalog:",
        *[f"    {p}: {_PKG}.adapters.local.catalog:LocalCatalogAdapter" for p in ("local", "gcp")],
        f"    onprem: {_PKG}.adapters.onprem.catalog:OnPremCatalogAdapter",
        "  baseline_store:",
        *[
            f"    {p}: {_PKG}.adapters.local.baseline_store:LocalBaselineStore"
            for p in ("local", "gcp")
        ],
        f"    onprem: {_PKG}.adapters.onprem.baseline_store:OnPremBaselineStore",
    ]
)
...
def test_a_malformed_assertion_from_a_lan_peer_answers_401_and_never_500(
    ...
    with TestClient(module.app, client=(LAN_PEER, 50000)) as client:
        resp = client.post(
            "/v1/certify",
            json={"dataset_id": "customer_master"},
...
def test_a_verified_assertion_is_served_so_the_refusals_above_mean_something(
    ...
    with TestClient(module.app, client=(LAN_PEER, 50000)) as client:
        resp = client.post(
            "/v1/certify",
            json={"dataset_id": "marketing_events"},
            headers={_IAP_ASSERTION_HEADER: signed_assertion()},
        )

...
def test_the_demo_data_is_obviously_fictional() -> None:
    datasets = (demo.CERTIFIED_DATASET, demo.REVIEW_DATASET, demo.UNCERTIFIED_DATASET)
    assert len(set(datasets)) == 3, "the arc needs three distinct fixture datasets"
    blob = " ".join(datasets) + demo.ACTOR + demo.TENANT + demo.PLANTED_NRIC
    for real_looking in ("@gmail.", "@outlook.", "@yahoo."):
        assert real_looking not in blob
    assert demo.ACTOR.endswith(".example"), "the actor should use an .example domain"
    # A synthetic national-id shape that exists only to prove the masking control fired.
...
def test_the_renderer_escapes_content_rather_than_interpolating_it() -> None:
    state = {
        ...
        "region": "r",
        "step_count": 1,
        "totals": {"datasets": 0, "escalated": 0, "routed": 0, "chain_ok": True},
        "steps": [
            {
                "key": "k",
                "label": "l",

...
from data_quality_governance.adapters.local.audit import (
    LocalAuditAdapter,
)
...
_ANCHOR_ENV = "DATAQUALITY_AUDIT_ANCHOR"
...
def _record_three(settings: Settings) -> LocalAuditAdapter:
    ...
    for dataset_id in _DATASETS:
        service.certify(dataset_id, actor=sample_cases.ACTOR, tenant=sample_cases.TENANT)
    return audit
...
def test_an_append_after_truncation_cannot_relaunder_the_anchor(tmp_path: Path) -> None:
    ...
    with pytest.raises(AuditChainError):
        build_certification_service(build_container(settings)).certify(
            sample_cases.CERTIFIED_DATASET, actor=sample_cases.ACTOR, tenant=sample_cases.TENANT
        )

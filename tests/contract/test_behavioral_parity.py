...
from data_quality_governance.adapters._review_payload import (
    result_to_review,
)
...
def test_the_payload_that_reaches_the_wire_is_redacted_whichever_family_built_it() -> None:
    ...
    result = sample_cases.escalating_scorecard()
    payload = repr(
...
def test_the_whole_pipeline_answers_on_local_and_fails_fast_on_onprem() -> None:
    ...
    result = build_certification_service(local).certify(
        sample_cases.REVIEW_DATASET, actor=sample_cases.ACTOR, tenant=sample_cases.TENANT
    )
    ...
    with pytest.raises(NotImplementedError):
        build_certification_service(onprem).certify(
            sample_cases.REVIEW_DATASET, actor=sample_cases.ACTOR, tenant=sample_cases.TENANT
        )

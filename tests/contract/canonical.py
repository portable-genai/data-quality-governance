...
from data_quality_governance.domain.kernel import (
    AuditEvent,
    Citation,
    Decision,
    Severity,
...
CANONICAL_EVENT = AuditEvent(
    ...
    citations=(Citation(source_id="pii:marketing_events", title="PII", snippet="sensitive"),),
)

#: The escalated scorecard every review-router implementation is handed (rule R8's payload).
CANONICAL_RESULT = sample_cases.escalating_scorecard()

#: The canonical dataset every warehouse/catalog/baseline implementation is asked about.
CANONICAL_DATASET = sample_cases.CERTIFIED_DATASET
...
def _warehouse_invoke(adapter: Any) -> Any:
    return adapter.metadata(CANONICAL_DATASET)


def _warehouse_answered(_adapter: Any, result: Any) -> bool:
    return getattr(result, "dataset_id", "") == CANONICAL_DATASET and bool(result.schema)


def _catalog_invoke(adapter: Any) -> Any:
    return adapter.owner(CANONICAL_DATASET)


def _catalog_answered(_adapter: Any, result: Any) -> bool:
    return isinstance(result, str) and "@" in result


def _baseline_invoke(adapter: Any) -> Any:
    return adapter.get_baseline(CANONICAL_DATASET, tenant=sample_cases.TENANT)


def _baseline_answered(_adapter: Any, result: Any) -> bool:
    return bool(result)
...
CANONICAL_CALLS: dict[str, PortCase] = {
    ...
    "warehouse": PortCase(
        ...
        # The lazy `google.cloud` import is the first thing the managed warehouse does.
        managed_refusal=(ImportError,),
        detail="return dataset metadata and the live schema",
    ),
    "catalog": PortCase(
        invoke=_catalog_invoke,
        answered=_catalog_answered,
        managed_refusal=(ImportError,),
        detail="return a dataset's owner",
    ),
    "baseline_store": PortCase(
        invoke=_baseline_invoke,
        answered=_baseline_answered,
        managed_refusal=(ImportError,),
        detail="return the tenant-scoped baseline schema",
    ),
    "tracer": PortCase(

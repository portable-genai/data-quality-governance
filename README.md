# Data Quality and Governance Agent (H4)

Deterministic DQ, freshness, drift and PII checks with a cited scorecard.
...
data_quality_governance triage "Acme (FICTIONAL)" "urgent data breach"
```
...
One env var, `DATAQUALITY_PROFILE`, selects the adapter family:
...
`DATAQUALITY_ALLOW_INSECURE_DEMO=1` says otherwise. The guard reads the identity
BINDING to decide that, never a service credential: setting
`DATAQUALITY_S2S_TOKEN` closes the S2S routes and does not open anything else.
See `docs/runbook.md`.
...
**Name the profile.** `DATAQUALITY_PROFILE` has no default. Leaving it unset is

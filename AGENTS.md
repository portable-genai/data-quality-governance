...
Data Quality and Governance Agent (H4): Deterministic DQ, freshness, drift and PII checks with a cited scorecard.
Rendered from `hex-service-template`, so it starts at reference parity rather than converging
toward it. Package `data_quality_governance`, environment prefix
`DATAQUALITY`, region `asia-southeast1`.
...
- `src/data_quality_governance/domain/` is PURE stdlib. No web framework, no cloud SDK,
...
- **The exposure guard is derived from the IDENTITY BINDING, and from nothing else.** An end-user
  ...
  `DATAQUALITY_S2S_TOKEN` may never enter that decision: it authenticates a
...
- **The one adapter that declares `VERIFIED` must EARN it, and it is the one adapter that may not
  ...
  configured `DATAQUALITY_IAP_AUDIENCE`, three-state: unset or emptied REFUSES,
...
- **One profile read, three states, refused at import.** `config.PROFILE_CHOICE` resolves once at
  ...
  `DATAQUALITY_PROFILE` (mentioning it in a refusal message is wanted, reading

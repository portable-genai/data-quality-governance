...
One env var (``DATAQUALITY_PROFILE``) selects the adapter family for every
...
from .ports.baseline_store import BaselineStorePort
from .ports.catalog import CatalogPort
...
_PROFILE_ENV = "DATAQUALITY_PROFILE"
_SETTINGS_ENV = "DATAQUALITY_SETTINGS"
...
class ProfileChoice:
    ...
    The variable is ``DATAQUALITY_PROFILE``, named once in
    :data:`_PROFILE_ENV`.
    ...
    ``os.environ.get("DATAQUALITY_PROFILE", "local")``: that fallback reads an
...
DEFAULT_BINDINGS: dict[str, dict[str, str]] = {
    ...
    "warehouse": {
        "local": f"{_PKG}.adapters.local.warehouse:LocalWarehouseAdapter",
        "gcp": f"{_PKG}.adapters.gcp.warehouse:CloudWarehouseAdapter",
        "onprem": f"{_PKG}.adapters.onprem.warehouse:OnPremWarehouseAdapter",
    },
    "catalog": {
        "local": f"{_PKG}.adapters.local.catalog:LocalCatalogAdapter",
        "gcp": f"{_PKG}.adapters.gcp.catalog:CloudCatalogAdapter",
        "onprem": f"{_PKG}.adapters.onprem.catalog:OnPremCatalogAdapter",
    },
    "baseline_store": {
        "local": f"{_PKG}.adapters.local.baseline_store:LocalBaselineStore",
        "gcp": f"{_PKG}.adapters.gcp.baseline_store:CloudBaselineStore",
        "onprem": f"{_PKG}.adapters.onprem.baseline_store:OnPremBaselineStore",
    },
}
...
def _read_settings_file(path: Path | None = None) -> dict[str, Any]:
    ...
    An EXPLICIT path (argument or ``DATAQUALITY_SETTINGS``) that does not
...
REVIEW_ROUTING_ENV = "DATAQUALITY_REVIEW_ROUTING"
...
class Container:
    ...
    def catalog(self) -> CatalogPort:
        adapter = self._bind("catalog")
        assert isinstance(adapter, CatalogPort)
        return adapter

    @cached_property
    def baseline_store(self) -> BaselineStorePort:
        adapter = self._bind("baseline_store")
        assert isinstance(adapter, BaselineStorePort)
        return adapter

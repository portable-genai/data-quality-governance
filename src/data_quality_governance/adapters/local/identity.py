...
came from the environment) ``DATAQUALITY_PROFILE`` must have been set rather
...
class LocalIdentityAdapter:
    ...
    def __init__(self, settings: Settings) -> None:
        ...
        if not settings.profile_explicit:
            raise LocalPersonaProfileError(
                "DATAQUALITY_PROFILE is not set, so the local profile was "
                "inherited rather than chosen; seeded dev personas grant their groups and "
                "tenant with no authentication and are refused. Set "
                "DATAQUALITY_PROFILE=local deliberately for a dev or demo "
                "run, or DATAQUALITY_PROFILE=gcp for a real deployment."
            )

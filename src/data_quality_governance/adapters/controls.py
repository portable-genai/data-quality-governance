...
(``DATAQUALITY_REVIEW_ROUTING=off``), the container binds :class:`DisabledReviewRouter` instead of
the profile's class. It satisfies the port and submits nothing, and the container logs the
posture at startup.

**Recording wrapper.** Every caller that hands a result to the router (the certify route, the
...
failure is logged and absorbed here rather than failing an already-certified, already-audited
scorecard, but it is never invisible: the caller reports ``failed`` and an empty reference,

"""FastAPI application for Data Quality and Governance Agent (H4).
...
``DATAQUALITY_PROFILE`` is NO CHOICE rather than a silent ``local``. Every
...
``DATAQUALITY_S2S_TOKEN`` is wrong in the most dangerous direction:
...
from ..domain.errors import CrossTenantError
...
_INSECURE_DEMO_ENV = "DATAQUALITY_ALLOW_INSECURE_DEMO"
_CORS_ORIGINS_ENV = "DATAQUALITY_CORS_ORIGINS"
_ALLOWED_CALLERS_ENV = "DATAQUALITY_S2S_ALLOWED_CALLERS"
_AUDIENCE_ENV = "DATAQUALITY_S2S_AUDIENCE"
_API_HOST_ENV = "DATAQUALITY_API_HOST"
...
def require_service_caller(request: Request) -> None:
    ...
    stays OPEN when ``DATAQUALITY_S2S_TOKEN`` is unset (loopback dev with zero
    secrets), so an UNSET ``DATAQUALITY_PROFILE`` must never be allowed to
    ...
    ``DATAQUALITY_ALLOW_INSECURE_DEMO=1``.
    """
    if not _request_choice(request).service_auth_configured:
        raise HTTPException(
            ...
            detail=(
                ...
                "DATAQUALITY_PROFILE is not set, so no authentication scheme "
                "has been chosen"
            ),
        )
...
_SERVICE_NAME = "data-quality-governance"
...
_APP_TITLE = "Data Quality and Governance Agent"
...
#
# Note what is NOT in this expression: DATAQUALITY_S2S_TOKEN. A service
...
@app.post("/v1/certify", response_model=ScorecardResponse, tags=["artifacts"])
def certify(
    request: CertifyRequest,
    principal: Annotated[Principal, Depends(get_principal)],
) -> ScorecardResponse:
    """Certify a dataset; the audit actor is the verified principal, never the request body.
    ...
    result = service.certify(request.dataset_id, actor=principal.actor, tenant=principal.tenant)
    # The hand-off never fails an already-certified, already-audited scorecard; the response says
    ...
    return ScorecardResponse.from_domain(
        result, review_ref=review_ref, review_routing=routing.outcome.value
    )


@app.get(
    "/v1/certification/{dataset_id}",
    response_model=CertificationResponseModel,
    tags=["artifacts"],
)
def certification_status(
    ...
    """The narrow certification-status wire H1 consumes, authorised against the caller's tenant.

    A dataset certified only under a different tenant is 403, never 404: answering 404 would leak
    whether the dataset exists to a tenant with no right to know. A dataset this tenant has not
    certified yet is 404.
    """
    service = build_certification_service(_container())
    try:
        response = service.response_for(dataset_id, tenant=principal.tenant)
    except CrossTenantError as exc:
        raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail=str(exc)) from exc
    if response is None:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
            detail=f"dataset {dataset_id!r} has no certification for this tenant yet",
        )
    return CertificationResponseModel.from_domain(response)

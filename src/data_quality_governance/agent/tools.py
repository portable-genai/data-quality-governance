...
DEFAULT_ACTOR = "data-quality-governance"
...
def certify_dataset(
    ...
    """Certify one dataset and route it for human review when it escalates.

    Runs the deterministic DQ, freshness, drift and PII engines over the dataset, folds a cited
    certification scorecard, writes an already-redacted audit event, and, when the outcome
    escalates (a decertification or a sensitive-category PII finding), submits the scorecard to
    the human-review console (rule R8).

    Args:
      dataset_id: The dataset to certify.
    ...
    Returns:
      ...
      only when the scorecard did not escalate.
    """
    container = _container(settings)
    result = build_certification_service(container).certify(dataset_id, actor=actor, tenant=tenant)
    review_ref = ""
...
TOOL_FUNCTIONS = (certify_dataset, verify_audit_trail)

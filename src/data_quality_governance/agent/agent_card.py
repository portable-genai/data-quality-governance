...
class AgentCard:
    ...
    provider: str = "data-quality-governance"
...
SKILLS: tuple[AgentSkill, ...] = (
    AgentSkill(
        id="certify_dataset",
        name="Dataset certification",
        description=(
            "Profile a dataset and run deterministic data-quality, freshness, schema-drift and "
            "PII-classification checks into a cited certification scorecard, then ROUTE it to "
            "human review when it escalates (rule R8). Every verdict and number is computed by "
            "pure stdlib code, never by a model."
        ),
    ),
    AgentSkill(
...
_DESCRIPTION = " ".join(
    (
        "Data Quality and Governance Agent",
        "(H4).",
...
def _resolve_url(settings: Settings) -> str:
    ...
    return f"https://data-quality-governance.{settings.region}.internal.example/a2a"

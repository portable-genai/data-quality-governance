...
| `rename_fork.py` | Not a demo script: the one-pass mechanical rebrand a FORK runs once, rewriting the package name, the console-script name, the `DATAQUALITY` env prefix, the Terraform `name_prefix` stem and the distribution id across the tree. It is listed here because it is stdlib-only and offline like everything else in this directory, and because the index test holds this table equal to the files on disk. Preview with `--dry-run`, apply with `--yes`; see [`../docs/ADOPTING.md`](../docs/ADOPTING.md) for the human decisions it deliberately leaves to you. |

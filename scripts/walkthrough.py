...
def _check_redaction(state: dict[str, Any]) -> list[str]:
    ...
    if facts.get("planted_identifier_leaked"):
        problems.append("the planted identifier survived the masking control")
    if facts.get("status") != "uncertified":
        problems.append("the fail-closed dataset was not uncertified: " + str(facts.get("status")))
    return problems
...
def _check_audit(state: dict[str, Any]) -> list[str]:
    ...
    if int(facts.get("entries", 0)) < 3:
        problems.append("fewer audit records than datasets certified")
    return problems

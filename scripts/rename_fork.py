...
Rewrites the python package name, the console-script name, the ``DATAQUALITY`` env-var prefix,
...
Markdown prose alone (pass ``--include-docs`` to sweep it too).
    ...
    # Apply it:
    python scripts/rename_fork.py --package acme_dq --cli acme-dq \
        --env-prefix ACME --resource acme-dq --yes
...
# --------------------------------------------------------------------------------------- #
_OLD_PACKAGE = "data_quality_governance"
_OLD_CLI = "data_quality_governance"
_OLD_ENV_STEM = "DATAQUALITY"
_OLD_RESOURCE = "h4-svc"
_OLD_DIST = "data-quality-governance"
...
def _rules(args: argparse.Namespace) -> list[tuple[str, str, str]]:
    ...
    return [
        ...
        # The env-var prefix. Matched WITHOUT the trailing underscore so both DATAQUALITY_PROFILE
...
def main() -> int:
    ...
    parser.add_argument("--cli", required=True, help="new console-script name, e.g. acme-dq")
    parser.add_argument("--env-prefix", required=True, help="new env-var prefix stem, e.g. ACME")
    parser.add_argument(
        "--resource", required=True, help="new Terraform name_prefix stem, e.g. acme-dq"
    )
    parser.add_argument(

...
below drives the actual certification service, the actual deterministic engines, the actual
...
* **It is replayable.** Same fixture, same verdicts, every time, because the consequential
  decision is deterministic. That is what makes it safe to run live.
...
from data_quality_governance.domain.pii import (
    JURISDICTIONS,
    PII_PATTERNS,
)
...
SERVICE_NAME = "Data Quality and Governance Agent"
CATALOG_ID = "H4"
REPOSITORY = "data-quality-governance"

# --------------------------------------------------------------------------------------- #
# Synthetic data. Fictional datasets and a planted identifier only.
# --------------------------------------------------------------------------------------- #
...
#: The fixture datasets, by intended verdict.
...
STEPS: tuple[Step, ...] = (
    ...
    Step(
        key="routine",
        label="A clean dataset: CERTIFIED, cited, NOT escalated",
        narration=(
            "A clean, fresh dataset. The verdict and every number come from deterministic code, "
            "never from a model. Nothing is routed for review, because manufacturing a review "
            "for a certified dataset trains reviewers to rubber-stamp."
        ),
    ),
    Step(
        key="escalation",
        label="A consequential finding: escalated AND routed (rule R8)",
        narration=(
            "A dataset with a sensitive-category PII column and schema drift. The verdict is "
            "conditional, and the sensitive finding is routed to the human-review console in the "
            "same call that produced it. The reference records where it went."
        ),
    ),
    Step(
        key="redaction",
        label="An UNCERTIFIED dataset, and masking BEFORE the audit write",
        narration=(
            "A stale partition with a duplicate id and null amounts: uncertified, fail-closed. "
            "The audit record is masked on the way in, so a planted identifier never reaches the "
            "immutable trail. Redacting afterwards would be too late."
        ),
    ),
    Step(
        key="review_queue",
        ...
        narration=(
            ...
            "against every configured jurisdiction, not only the one this dataset came from."
        ),
    ),
    Step(
        key="audit",
...
class DemoRun:
    ...
    def __init__(self, workdir: Path | None = None) -> None:
        ...
        self.service = build_certification_service(self.container)
    ...
    def _step_routine(self) -> Produced:
        return self._certify_panels(CERTIFIED_DATASET, expect_routing=False)

    def _step_escalation(self) -> Produced:
        return self._certify_panels(REVIEW_DATASET, expect_routing=True)

    def _step_redaction(self) -> Produced:
        panels, facts = self._certify_panels(UNCERTIFIED_DATASET, expect_routing=False)
        # The masking control the service applies before every audit write and every outbound
        # payload, shown on an independent planted identifier so the panel does not merely trust
        # the pattern pack to agree with itself.
        planted = f"owner contact NRIC {PLANTED_NRIC} on file"
        masked = redact(planted, PII_PATTERNS)
        recorded = str(self.container.audit.log.read_all()[-1]["redacted_summary"])
        leaked = PLANTED_NRIC in masked or PLANTED_NRIC in recorded
        panels.append(
            Panel(
                title="Redact before the write",
                rows=(
                    Row("Identifier in the source text", PLANTED_NRIC, "warn"),
                    Row("Same text after the masking control", masked, "ok"),
                    Row(
                ...
                note=(
                    ...
                    "late. The same pattern pack masks the audit summary and the outbound review."
                ),
    ...
    def _step_review_queue(self) -> Produced:
        ...
        findings = Panel(
            title="Findings",
            rows=(
                Row("Escalated scorecards", str(self.escalated)),
                Row(
                    "Routed to review",
                    str(self.routed),
    ...
    def _certify_panels(self, dataset_id: str, *, expect_routing: bool) -> Produced:
        result = self.service.certify(dataset_id, actor=ACTOR, tenant=TENANT)
        self.datasets += 1
        ...
        verdict = Panel(
            title="Certification: " + result.subject,
            rows=(
                Row(
                    "Status",
                    result.status.value,
                    "ok" if result.status.value == "certified" else "warn",
                ),
                Row("DQ pass ratio", str(result.pass_ratio)),
                Row("Worst severity", result.severity.value),
                ...
                Row("Certified metrics", ", ".join(result.certified_metrics) or "(none)"),
                Row("Attributed to", ACTOR),
            ),
            note=(
                "The verdict and every number are computed by pure stdlib engines and are "
                "replayable. A model would narrate this scorecard; it never produces it."
            ),
        ...
        checks = Panel(
            ...
            rows=tuple(
                Row(
                    f.rule_id,
                    ("PASS" if f.passed else "FAIL") + f" ({f.severity.value}) {f.observed}",
                    "ok" if f.passed else "bad",
                )
                for f in result.dq_findings
            )
            or (Row("checks", "none configured", "bad"),),
            note="Each finding cites its rule, column and the sampled evidence.",
        )
        signals = Panel(
            title="Explainable PII",
            rows=tuple(
                Row(
                    c.column,
                    f"{c.category.value} (score {c.score})",
                    "warn" if c.category.value == "sensitive" else "",
                )
                for c in result.pii
            ),
            note=(
                "Every column's category is the sum of name, pattern and shape signals "
                "an auditor can recompute."
            ),
        )
        facts = {
            "dataset_id": result.subject,
        ...
        return [verdict, checks, signals], facts
...
def _exit_warehouse(container: Any) -> Any:
    return container.warehouse.metadata(CERTIFIED_DATASET)


def _exit_catalog(container: Any) -> Any:
    return container.catalog.owner(CERTIFIED_DATASET)


def _exit_baseline(container: Any) -> Any:
    return container.baseline_store.get_baseline(CERTIFIED_DATASET, tenant=TENANT)
...
def _exit_audit(container: Any) -> Any:
    return container.audit.record(
        kernel.AuditEvent(
            ...
            redacted_summary="marketing_events: conditionally_certified",
        )
    )


def _exit_review(container: Any) -> Any:
    scorecard = models.DatasetScorecard(
        subject=REVIEW_DATASET,
        status=models.CertificationStatus.CONDITIONAL,
        decision=kernel.Decision.ESCALATED,
        severity=kernel.Severity.HIGH,
        summary=REVIEW_DATASET + ": conditionally_certified",
        requires_human_review=True,
        pass_ratio=1.0,
        citations=(kernel.Citation(source_id="pii:x", title="PII", snippet="sensitive"),),
    )
    return container.review_router.route(scorecard, maker=ACTOR, tenant=TENANT)
...
EXIT_CALLS: dict[str, Callable[[Any], Any]] = {
    ...
    "catalog": _exit_catalog,
    "baseline_store": _exit_baseline,
}

...
`requires_human_review` and are ROUTED to the **Hrz7** human-review console in the same request
...
switch, an eval that scores the live model, and prompt-injection screening through the Hrz1
guardrail).
...
report. `--mode gate` delegates the promotion verdict to the sibling **Hrz4** authority and refuses
to run off the managed profile. What is still open: this repo's metric bundle is not yet registered
with Hrz4, so gate mode has no authority to ask (P-08 and R5).
...
against EVERY jurisdiction's rows before a review leaves the process, because the Hrz7 console is a
shared sink and Hrz7 redacts again on arrival (defence in depth). A tool result is masked before it
...
or latency controls, because there is no model call to route or cache) and R6 (no Rsk3 intake
...
Hrz4), R1 (no guardrail port), R2 (the audit half is local, not yet in the shared Hrz5 sink), R4
(the agent card is served but not registered with Hrz3) and tenant isolation (no object-level

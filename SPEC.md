# SPEC: Data Quality and Governance Agent (H4)
...
- **Maker-checker (P-06) and routing (R8)**: a HIGH/CRITICAL result sets
  ...
  `DATAQUALITY_REVIEW_ROUTING=off` is the stated way to run without it.
- **Profile**: resolved ONCE, at import, into a `ProfileChoice` and never a bare string. Three
  states of `DATAQUALITY_PROFILE`: UNSET is NO CHOICE (the SDK-free adapters
